...
// 		System.out.println("SecondAddXuqiu界面的btnname1：" + btnname1);
		...
		System.out.println("jsshresult界面的Inamenum：" + Inamenum);
		String Iname = session.getAttribute("Iname").toString();
		System.out.println("jsshresult界面的Iname：" + Iname);
		
		String str_insertjssh="insert into jssh values('" + Iname + "','" + Inamenum + "','" + "技术审核通过" + "','" + "" + "','" + str_date + "')";
		System.out.println("将通过结果插入jssh的插入语句："+str_insertjssh);
		int j=db.executeUpdate(str_insertjssh);
		System.out.println("将通过结果插入jssh的i："+j);
		
		String str = "update xuqiu set Iresult=" + "'" + "技术审核通过" + "'"+" where Inamenum=" + "'" + Inamenum + "'";
		System.out.println("修改技术审核需求结果的修改语句："+str);
		int i=db.executeUpdate(str);
		System.out.println("修改技术审核需求结果的i："+i);
		if (i == 1) {
// 			if (result.equals("已保存"))
// 				out.println("<script language='javaScript'>alert('保存成功，点击确定返回主页面！');</script>");
// 			else if (result.equals("已提交"))
				out.println("<script language='javaScript'>alert('技术审核通过提交成功，点击确定返回审核管理页面！');</script>");
